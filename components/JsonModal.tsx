...
export const JsonModal: React.FC<JsonModalProps> = ({ isOpen, onClose, data = '', isImport = false, onImport, language }) => {
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl flex flex-col h-[70vh] border-4 border-slate-100 animate-in fade-in zoom-in duration-200 scale-95 sm:scale-100">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
                <h2 className="text-xl font-extrabold text-slate-800">
                    {isImport ? t.importTitle : t.shareTitle}
                </h2>
                <p className="text-xs font-bold text-slate-400 uppercase tracking-wide">{t.jsonFormat}</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="flex-1 p-6 overflow-hidden bg-slate-50/50 flex flex-col relative">
          <textarea 
            ...
            placeholder={isImport ? t.pasteLabel : ""}
